...
?>
		...
		<div class="panel panel-primary">
            <div class="panel-heading">
              <h1 class="panel-title">Manage SEBS Users</h1>
            ...
            <div class="panel-body" style="max-height: 91%;overflow-y: scroll;">
            	<div class="well well-sm" style="text-align:right;">
              		...
              		<button class="btn btn-success modal-trigger" data-toggle="modal" data-target="#remotecatcher" data-rh="sebs.manage.users.add&type=student">Create New Student</button>
            					...
            					echo('
            						<div class="btn-group" style="float:left;">
  						<a href="index.php?do=sebs.manage.users&filter=students" class="btn btn-default">Students</a>
  						<a href="index.php?do=sebs.manage.users&filter=teachers" class="btn btn-info disabled">Staff</a>
			  		</div>
              		<button class="btn btn-success modal-trigger" style="" data-toggle="modal" data-target="#remotecatcher" data-rh="sebs.manage.users.add&type=teacher">Add Staff Member</button>
            					');
            				} else {
            				if($mobile) {
            					echo('<p style="text-align:left;display:inline-block;margin-right:5px;">Select a user type to use filter functions <i class="fa fa-arrow-down"></i></p>');
            				} else {
            					echo('<p style="text-align:left;display:inline-block;margin-right:5px;">Select a user type to use filter functions <i class="fa fa-arrow-right"></i></p>');
            				}
            					echo('
            						<div class="btn-group">
  						<a href="index.php?do=sebs.manage.users&filter=students&query='.$query.'" class="btn btn-default">Students</a>
  						<a href="index.php?do=sebs.manage.users&filter=teachers&query='.$query.'" class="btn btn-default">Staff</a>
			  		</div>
            					');
            				}
            			?>
            	...
            	<table id="sebs.manage.users.list" data-filter="<?php echo($filter);?>" class="table table-striped table-hover table-condensed">
				</table>
            </div>
          </div>
        </div>
	  ...
	  document.getElementById('sebs.manage.users.list').innerHTML = "<center><h4>Fetching Request...</h4></center>";
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
		...
		if (empdata["status_code"] == "success") {
			...
			tableHtml += "</tbody>";
			document.getElementById('sebs.manage.users.list').innerHTML = tableHtml;
			readyModal();
		} else {
  ...
  xhttp.open("GET", "remote-json.php?do=sebs.manage.users.list&query="+qu+"&filter=<?php echo($filter);?>&filter_house="+hn+"&filter_yrgrp="+yg, true);
  xhttp.send();
  }
