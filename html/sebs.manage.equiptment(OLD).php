...
?>
		...
		<div class="panel panel-primary">
            ...
            <div class="panel-body" style="max-height: 91%;overflow-y: scroll;">
            	...
            	<table id="sebs.manage.equiptment.list" data-filter="N/A" class="table table-striped table-hover table-condensed">
				</table>
            </div>
          </div>
        </div>
	  ...
	  <script>
	  function getData(qu,cg,sp) {
	  document.getElementById('sebs.manage.equiptment.list').innerHTML = "<center><h4>Fetching Request...</h4></center>";
    ...
    if (this.readyState == 4 && this.status == 200) {
		...
		if (empdata["status_code"] == "success") {
			if( /AndUser IDroid|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
				var tableHtml = "<thead class=\'\'><tr><th>Sport</th><th>Category</th><th>Item Name</th><th>Actions</th></tr></thead><tbody>";
			} else {
				var tableHtml = "<thead class=\'\'><tr><th>Sport</th><th>Category</th><th>Item Name</th><th>Actions</th></tr></thead><tbody>";
			}
			for(i = 0; i < empdata["DATA"].length; i++){
				...
				tableHtml += "<tr style='cursor:pointer;' data-item_code=\'"+empdata["DATA"][i]["item_code"]+"\'><td "+rowAction+" class=\'text-warning\'>";
				...
				tableHtml += "<button class='btn btn-danger modal-trigger' style='width:50%;' data-toggle='modal' data-target='#remotecatcher' data-rh='sebs.manage.equiptment.remove&username="+empdata["DATA"][i]["uuid"]+"'><span class='fa fa-trash'><dead>Remove User</dead></span></button>";
				tableHtml += "</div></td></tr>";
				
			}
			tableHtml += "</tbody>";
			document.getElementById('sebs.manage.equiptment.list').innerHTML = tableHtml;
			readyModal();
		} else {
  ...
  xhttp.open("GET", "remote-json.php?do=sebs.manage.equiptment.list&query="+qu+"&filter=<?php echo($filter);?>&filter_category="+cg+"&filter_sport="+sp, true);
  xhttp.send();
  }
  getData("","","");
	  </script>
