...
if ($intent == "setup") {
	...
	$sql = "DELETE FROM items WHERE item_code=".$itemcode;
	if($result = $conn->query($sql)){
		$sql = "DELETE FROM loans WHERE item_code=".$itemcode;
		if($result = $conn->query($sql)){
			$empty = array("status_code"=>"success","status_desc"=>"Item $itemcode has been deleted and all accociated loans!","intent"=>$intent,"DATA"=>array());
    		$json = json_encode($empty);
			echo($json);
		} else {
		...
		$empty = array("status_code"=>"error","status_desc"=>"Err1: No Item Exists!","DATA"=>array());
    	$json = json_encode($empty);
		echo($json);
	}
